...
(*
  ...
  TMPHexEditorEx v 07-10-2010<br>
...
uses
   ...
   ActiveX,
   Graphics,
   Printers, {$IFDEF DELPHI6UP}{$IFDEF USESTDACTIONS}StdActns,
   Clipbrd, {$ENDIF}{$ENDIF}
   ShlObj,
   Menus;
...
type
   ...
   *)
   ...
   private
      ...
      function OLEHasSupportedFormat(const dataObj: IDataObject; const Formats: array of TClipFormat; var Format: TClipFormat): boolean;
      function GetMyOLEFormats: TClipFormats;
      procedure WMDestroy(var message: TWMDestroy); message WM_DESTROY;
      procedure SetPrintOptions(const Value: TMPHPrintOptions);

      function PrintToCanvas(ACanvas: TCanvas; const APage: integer; const AMargins: TRect): integer;
   ...
   protected
      ...
      function CanCreateUndo(const aKind: TMPHUndoFlag; const aCount, aReplCount: Int64): boolean; override;
      ...
      function SupportsOLEData(const dataObj: IDataObject; const grfKeyState: longint; const pt: TPoint; var dwEffect: longint; const Operation: TMPHOLEOperation): HRESULT;
      // @exclude(insert ole-dropped data)
      function InsertOLEData(const dataObj: IDataObject; const grfKeyState: longint; const pt: TPoint; var dwEffect: longint; const Operation: TMPHOLEOperation): HRESULT;
      // @exclude(modify drageffect depending on key states and data format)
      function ModifyOLEDropEffect(const grfKeyState: longint; const pt: TPoint; var dwEffect: longint): HRESULT;
   ...
   public
      ...
      procedure WriteBuffer(const ABuffer; const AIndex, ACount: Int64); override;
      ...
      property CreateUndoOnUndoUpdate: boolean read FCreateUndoOnUndoUpdate write FCreateUndoOnUndoUpdate;
      ...
      *)
      ...
      property BookMarksAsString: string read GetBookmarksAsString write SetBookMarksAsString;
      {$IFDEF DELPHI6UP}
      // get set properties as text (for storing in registry, ini-file);
      property PropertiesAsString: string read GetPropertiesAsString write SetPropertiesAsString;
      {$ENDIF}
   published
      { Published-Deklarationen }
      // create a backup on save ? (see also @link(BackupExtension))
      property CreateBackup: boolean read FCreateBackups write FCreateBackups default True;
      ...
      *)
      ...
      property ClipboardAsHexText: boolean read FClipboardAsHexText write FClipboardAsHexText default False;
      // flush or empty clipboard at shutdown
      property FlushClipboardAtShutDown: boolean read FFlushClipboardAtShutDown write FFlushClipboardAtShutDown default False;
      // do we support other formats than CF_MPHEXEDITOR and CF_HDROP?
      property SupportsOtherClipFormats: boolean read FSupportsOtherClipFormats write FSupportsOtherClipFormats default True;
      // print/preview options, see @link(TMPHPrintOptions)
      property PrintOptions: TMPHPrintOptions read FPrintOptions write SetPrintOptions;
      ...
      property UseEditorFontForPrinting: boolean read FUseEditorFontForPrinting write FUseEditorFontForPrinting default True;
      ...
      *)
      property OffsetPopupMenu: TPopupMenu read GetOffsetPopupMenu write SetOffsetPopupMenu;
      // auto-zoom on mouse wheel?
      property ZoomOnWheel: boolean read FZoomOnWheel write FZoomOnWheel default True;
      (* this event is called when @link(PropertiesAsString) is read or written.
        (see @link(TMPHQueryPublicPropertyEvent))
      *)
      property OnQueryPublicProperty: TMPHQueryPublicPropertyEvent read FOnQueryPublicProperty write FOnQueryPublicProperty;
      // @exclude(inherited)
      property Align;
      // @exclude(inherited)
      property Anchors;
...
procedure MPHSetHexClipboardData(Data: Pointer; DataSize: integer; ScrapFileName: ShortString; TextAsHex, SwapNibbles: boolean);
...
var
   ...
   CF_MPHEXEDITOR, CF_RTF, CF_FILECONTENTS, CF_PERFORMEDDROPEFFECT, CF_LOGICALPERFORMEDDROPEFFECT, CF_FILEDESCRIPTOR, CF_HTML, CF_REGEDIT_HEXDATA: TClipFormat;
...
type
   ...
   public
      constructor Create(Data: Pointer; DataSize: integer; ScrapFileName: ShortString; TextAsHex, SwapNibbles: boolean);
      constructor CreateFromStream(Stream: TStream; Position, DataSize: integer; ScrapFileName: ShortString; TextAsHex, SwapNibbles: boolean);
      procedure BeforeDestruction; override;
   ...
   private
      FMargins: TRect;
      FHeaders, FPrintHeaders: TMPHPrintHeaders;
   ...
   public
      constructor Create(AEditor: TMPHexEditorEx; ACanvas: TCanvas; AFlags: TMPHPrintFlags; AMargins: TRect; AHeaders: TMPHPrintHeaders);
...
function GetIDataObjectData(const dataObj: IDataObject; const Format: TClipFormat; out medium: TStgMedium): HRESULT;
var
   LobjEnum: TFormatEnum;
begin
   LobjEnum := TFormatEnum.Create(dataObj);
   try
      if not LobjEnum.HasFormat(Format) then Result := E_FAIL
      else Result                                   := dataObj.GetData(LobjEnum.GetFormatETC(Format), medium);
   finally
      LobjEnum.Free;
   end;
end;
...
function GetSomeData(const PData: Pointer; const HGlobal: THandle; Format: TClipFormat; const DataSize: integer; const UnicodeBigEndian: boolean): string;
var
...
begin
   ...
   if (Format = CF_RTF) or (Format = CF_HTML) then Format := CF_TEXT;

   if Format = CF_MPHEXEDITOR then begin
      with PClipData(PData)^ do
         if (Signature = CLIP_SIG) and (Version = CLIP_VER) then SetString(Result, Data, Size)
   end
   else if Format = CF_REGEDIT_HEXDATA then begin
      with PRegEditHexData(PData)^ do SetString(Result, Data, Size);
   end
   else
      case Format of
         ...
         CF_UNICODETEXT: begin
               ...
               if UnicodeBigEndian then begin
                  for LIntLoop := 1 to Length(LWStrTemp) do SwapWideChar(LWStrTemp[LIntLoop]);
               end;
               {$WARNINGS OFF}
               // don't convert, get wide data as is
               SetString(Result, PChar(LWStrTemp), Length(LWStrTemp) * (sizeof(widechar) div sizeof(char)));
               {$WARNINGS ON}
            end;
         CF_LOCALE: begin
         ...
         CF_PALETTE: begin
               ...
               LIntTemp := 0;
               if (GetObject(HGlobal, sizeof(LIntTemp), @LIntTemp) <> 0) and (LIntTemp > 0) then begin
                  with LRecPalette do begin
                     palVersion    := $0300;
                     palNumEntries := LIntTemp;
                     GetPaletteEntries(HGlobal, 0, LIntTemp, palPalEntry);
                  end;
                  SetLength(Result, sizeof(TLogPalette) + ((LIntTemp - 1) * sizeof(TPaletteEntry)));
         ...
         CF_BITMAP: begin
               ...
               try
                  LbmpTemp.Handle := CopyImage(HGlobal, IMAGE_BITMAP, 0, 0, LR_COPYRETURNORG);
...
function SelectClipFormat(const Formats: array of TClipFormat; var Format: TClipFormat; var TextIsHexData: boolean): boolean;
var
   ...
   with LfrmDialog do
      try
         ...
         try
            ...
            with LlbxFormats do begin
               Parent     := LfrmDialog;
               Align      := alTop;
               Height     := LfrmDialog.ClientHeight - 16 - LbtnCancel.Height - 8 - LcbxTextAsHex.Height;
               OnDblClick := ListDoubleClick;
               OnClick    := ListSelect;

               for LIntLoop   := low(Formats) to high(Formats) do begin
                  ...
                  case LWrdCurrent of
                     ...
                     else SetString(LStrFormatName, LszBuffer, GetClipboardFormatName(LWrdCurrent, LszBuffer, sizeof(LszBuffer)));
                        LStrFormatName := Trim(LStrFormatName);
                     end;
                  if LStrFormatName = '' then LStrFormatName := '(' + IntToRadix(LWrdCurrent, 10) + ')';
            ...
            if (ShowModal = mrOk) and (LlbxFormats.ItemIndex > -1) then begin
               Format := TClipFormat(LlbxFormats.Items.Objects[LlbxFormats.ItemIndex]);
               if Format in [CF_TEXT, CF_OEMTEXT] then TextIsHexData := LcbxTextAsHex.Checked;
               Result       := True;
               LAST_USED_CF := Format;
            end;
         finally
...
function QueryOLEFormat(const SupportedFormats: array of TClipFormat; const dataObj: IDataObject; var Format: TClipFormat; var TextIsHexData: boolean): boolean;
var
...
begin
   ...
   try
      ...
      if Length(SupportedFormats) > 0 then begin
         for LIntLoop := low(SupportedFormats) to high(SupportedFormats) do
            if LobjEnum.HasFormat(SupportedFormats[LIntLoop]) then begin
...
begin
   ...
   if not(csDesigning in ComponentState) then FDropTarget := TMPHDropTarget.Create(self); // not in delphi ide
end;
...
begin
   ...
   FPrintOptions.Free;
   FPrintFont.Free;
   if not(csDesigning in ComponentState) then FDropTarget.Free;
   inherited;
end;
...
begin
   ...
   Result                                 := (not(ReadOnlyView (* or NoSizeChange *) )) and Succeeded(OLEGetClipboard(LifData)) and (SupportsOLEData(LifData, 0, Point(0, 0), LIntEffect, oleClipboard) = S_OK);
   if Result and NoSizeChange then Result := DataSize > 0;
end;
...
begin
   ...
   if Result then begin
      WaitCursor;
      try
         FClipData := TMPHDataObject.CreateFromStream(DataStorage, Min(SelStart, SelEnd), SelCount, ExtractFileName(FileName), FClipboardAsHexText, SwapNibbles);
...
begin
   LIntEffect := DROPEFFECT_COPY;
   Result     := CanPaste and Succeeded(OLEGetClipboard(LifData)) and Succeeded(InsertOLEData(LifData, 0, Point(0, 0), LIntEffect, oleClipboard));
end;

// create an undo for a range of bytes

procedure TMPHexEditorEx.CreateRangeUndo(const aStart, aCount: integer; sDesc: string);
var
   bMod: boolean;
begin
   bMod := FModified;
   try
      if aCount < 1 then CreateUndo(ufKindAllData, 0, 0, 0, sDesc)
      else CreateUndo(ufKindReplace, aStart, aCount, aCount, sDesc);
   finally
      FModified := bMod;
   end;
end;
...
begin
   ...
   if FPaintUpdateCounter < 0 then FPaintUpdateCounter := 0;
   if FPaintUpdateCounter = 0 then Invalidate;
   Result := FPaintUpdateCounter;
end;
...
function TMPHexEditorEx.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): boolean;
begin
   if FZoomOnWheel and (Shift = [ssCtrl]) and (BytesPerRow > 1) then begin
      Result      := True;
      BytesPerRow := BytesPerRow - 1;
      Invalidate;
   end
   else if FZoomOnWheel and (Shift = [ssShift, ssCtrl]) and (Font.Size > 2) then begin
      Result    := True;
...
function TMPHexEditorEx.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): boolean;
begin
   if FZoomOnWheel and (Shift = [ssCtrl]) and (BytesPerRow < 256) then begin
      Result      := True;
      BytesPerRow := BytesPerRow + 1;
      Invalidate;
   end
   else if FZoomOnWheel and (Shift = [ssShift, ssCtrl]) then begin
      Result    := True;
      Font.Size := Font.Size + 1;
   end
...
begin
   ...
   case Key of
      ...
      Ord('C'): if (Shift = [ssCtrl]) and CanCopy then begin
            CBCopy;
         end;

      // CTRL+X: cut to clipboard
      Ord('X'): if (Shift = [ssCtrl]) and CanCut then begin
            CBCut;
         end;

      // CTRL+V: paste from clipboard
      Ord('V'): if (Shift = [ssCtrl]) and CanPaste then begin
            CBPaste;
         end;
      ...
      Ord('Z'): begin
            // undo
            if (Shift = [ssCtrl]) and CanUndo then begin
               Undo;
            end
            // redo
            else if (Shift = [ssShift, ssCtrl]) and CanRedo then begin
               Redo;
            end
         end;
   end;
...
begin
   ...
   if (FCreateBackups and Modified) and FileExists(FileName) then begin
      LStrBackup := FileName + FBackupFileExt;
      if FileExists(LStrBackup) and not DeleteFile(LStrBackup) then raise EMPHexEditor.CreateFmt(ERR_BACKUP_DELETE, [LStrBackup, SysErrorMessage(GetLastError)]);

      if (not MoveFile(PChar(FileName), PChar(LStrBackup))) then raise EMPHexEditor.CreateFmt(ERR_BACKUP_CREATE, [LStrBackup, SysErrorMessage(GetLastError)]);
   end;
end;
...
begin
   if not HasFile then raise EMPHexEditor.Create(ERR_NOFILE);
   SaveToFile(FileName);
end;

// prepare ole dragging

procedure TMPHexEditorEx.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: integer);
begin
   inherited;
   if FOleDragDrop and (Button = mbLeft) and MouseOverSelection and (not IsSelecting) then begin
      FOleStartDrag := True;
      FOleDragging  := False;
      FOleDragX     := X;
      FOleDragY     := Y;
...
begin
   ...
   if FOleDragDrop and (ssLeft in Shift) and (not FOleDragging) and FOleStartDrag and MouseOverSelection and (not IsSelecting) and ((Abs(X - FOleDragX) >= Mouse.DragThreshold) or (Abs(Y - FOleDragY) >= Mouse.DragThreshold)) then begin
      FOleStartDrag := False;
      FOleDragging  := True;
      FOleWasTarget := False;
      // start ole dragging
      try
         LobjData                               := TMPHDataObject.CreateFromStream(DataStorage, Min(SelStart, SelEnd), SelCount, ExtractFileName(FileName), FClipboardAsHexText, SwapNibbles);
         if not ReadOnlyView then LHrsOperation := DoDragDrop(LobjData, TMPHDropSource.Create, DROPEFFECT_COPY or DROPEFFECT_MOVE, LIntEffect)
         else LHrsOperation                     := DoDragDrop(LobjData, TMPHDropSource.Create, DROPEFFECT_COPY, LIntEffect);
         // if feedback has given via idataobject.setdata
         if LobjData.FHasDropEffect then LIntEffect := LobjData.FDropEffect;
         // unexcpected result
         if (LHrsOperation <> DRAGDROP_S_CANCEL) and (LHrsOperation <> DRAGDROP_S_DROP) then OleCheck(LHrsOperation)
         else if (LHrsOperation = DRAGDROP_S_DROP) and (LIntEffect = DROPEFFECT_MOVE) then begin
            // dragged to an other window
            if not FOleWasTarget then DeleteSelection
            else
...
procedure TMPHexEditorEx.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: integer);
begin
   if FHasDoubleClicked then begin
      MouseUpCanResetSel := False;
      FHasDoubleClicked  := False;
   end;
   inherited;
   if FOleDragging then begin
      FOleDragging  := False;
      FOleStartDrag := False;
   end;
end;
...
begin
   if Value <> FOleDragDrop then begin
      FOleDragDrop := Value;
      if ComponentState * [csLoading, csDesigning] = [] then FDropTarget.Active := Value;
   end;
end;
...
function TMPHexEditorEx.InsertOLEData(const dataObj: IDataObject; const grfKeyState: longint; const pt: TPoint; var dwEffect: longint; const Operation: TMPHOLEOperation): HRESULT;
var
   LRecStg: TStgMedium;
...
begin
   ...
   try
      // haupt-format?
      if ((FOleFormat[Operation] = CF_MPHEXEDITOR) or (FOleFormat[Operation] = CF_HDROP)) or QueryOLEFormat(GetMyOLEFormats, dataObj, FOleFormat[Operation], FClipboardAsHexText) then begin
         // je nach format daten konvertieren
         case FOleFormat[Operation] of
            CF_HDROP: if Succeeded(GetIDataObjectData(dataObj, FOleFormat[Operation], LRecStg)) then
                  try
                     // link: -> put all filenames
                     // copy: -> copy contents of first file
                     LIntLoop := DragQueryFile(LRecStg.HGlobal, cardinal(-1), nil, 0);
                     if LIntLoop > 0 then begin
                        for LIntLoop := 0 to Pred(LIntLoop) do begin
                           DragQueryFile(LRecStg.HGlobal, LIntLoop, LszBuf, sizeof(LszBuf));
                           Result := S_OK;
                           if dwEffect = DROPEFFECT_LINK then begin
                              LStrData := LStrData + StrPas(LszBuf) + #0;
                              LIntData := Length(LStrData);
                           end else begin
                              Result   := E_FAIL;
                              LfstFile := TFileStream.Create(LszBuf, fmOpenRead or fmShareDenyNone);
                              try
               ...
               if Succeeded(GetIDataObjectData(dataObj, FOleFormat[Operation], LRecStg)) then
                  try
                     if LRecStg.tymed in [TYMED_HGLOBAL, TYMED_MFPICT] then begin
                     ...
                     try
                        ...
                        if Assigned(LPtrLock) then GlobalUnlock(LRecStg.HGlobal);
                     end;
                  finally
                     ReleaseStgMedium(LRecStg);
                  end
         ...
         if (LStrData <> '') and (LIntData > 0) then begin
            // insert the data
            case Operation of
               oleDrop: begin
                     LIntPos                    := DropPosition;
                     if LIntPos < 0 then Result := E_FAIL
                     else begin
                        if FOleDragging and (dwEffect = DROPEFFECT_MOVE) then begin
                           FFixedFileSizeOverride := True;
                           try
                              // delete selection if we have moved data within ourself
                              FOleWasTarget := True;
                              if LIntPos > Min(SelStart, SelEnd) then Dec(LIntPos, SelCount);
                              DeleteSelection;
                              if LIntPos >= DataSize then Appendbuffer(@LStrData[1], LIntData, UNDO_DROPPED)
                              else InsertBuffer(@LStrData[1], LIntData, LIntPos, UNDO_DROPPED);
                           finally
                              FFixedFileSizeOverride := False;
                           end;
                        end else begin
                           if LIntPos >= DataSize then begin
                              if not NoSizeChange then Appendbuffer(@LStrData[1], LIntData, UNDO_DROPPED)
                           end else begin
                              if not NoSizeChange then begin
                                 if IsSelected(LIntPos) then ReplaceSelection(@LStrData[1], LIntData, UNDO_DROPPED)
                                 else InsertBuffer(@LStrData[1], LIntData, LIntPos, UNDO_DROPPED)
                              end else begin
                                 if (SelCount = 0) or (not IsSelected(LIntPos)) then Replace(@LStrData[1], LIntPos, LIntData, LIntData, UNDO_DROPPED)
                                 else ReplaceSelection(@LStrData[1], LIntData, UNDO_DROPPED)
                              end;
                           end;
                        end;
                     end;
...
function TMPHexEditorEx.SupportsOLEData(const dataObj: IDataObject; const grfKeyState: integer; const pt: TPoint; var dwEffect: integer; const Operation: TMPHOLEOperation): HRESULT;
begin
   Result := S_FALSE;
   if (not ReadOnlyView) and OLEHasSupportedFormat(dataObj, GetMyOLEFormats, FOleFormat[Operation]) then Result := S_OK;
   if FOleFormat[Operation] = CF_HDROP then
      if dwEffect = DROPEFFECT_MOVE then dwEffect := DROPEFFECT_LINK;
end;

function TMPHexEditorEx.OLEHasSupportedFormat(const dataObj: IDataObject; const Formats: array of TClipFormat; var Format: TClipFormat): boolean;
var
   LIntLoop: integer;
   LobjEnum: TFormatEnum;
begin
   ...
   try
      if Length(Formats) > 0 then
         for LIntLoop := low(Formats) to high(Formats) do
            if LobjEnum.HasFormat(Formats[LIntLoop]) then begin
...
function TMPHexEditorEx.ModifyOLEDropEffect(const grfKeyState: integer; const pt: TPoint; var dwEffect: integer): HRESULT;
begin
   Result := S_OK;
   if FOleDragging then begin
      if ReadOnlyView then dwEffect := DROPEFFECT_COPY
      else begin
         if BOOL(grfKeyState and MK_CONTROL) then dwEffect := DROPEFFECT_COPY
         else dwEffect                                     := DROPEFFECT_MOVE;
      end;
   end else begin
      if BOOL(grfKeyState and MK_SHIFT) and (not ReadOnlyView) then dwEffect := DROPEFFECT_MOVE
...
begin
   if FSupportsOtherClipFormats then SetLength(Result, 17)
   else SetLength(Result, 2);
   Result[0] := CF_MPHEXEDITOR;
   Result[1] := CF_HDROP;
   if FSupportsOtherClipFormats then begin
      Result[2]  := CF_TEXT;
      Result[3]  := CF_RTF;
...
begin
   inherited;
   if ComponentState * [csLoading, csDesigning] = [] then FDropTarget.Active := False;
end;
...
function TMPHexEditorEx.PrintToCanvas(ACanvas: TCanvas; const APage: integer; const AMargins: TRect): integer;
var
   LObjPrinter: TMPHCanvasPrinter;
   LSetFlags: TMPHPrintFlags;
begin
   if APage < 0 then raise EMPHexEditor.Create(ERR_INVALID_PAGE);
   WaitCursor;
   LSetFlags := FPrintOptions.Flags;
   try
      if SelCount = 0 then Exclude(LSetFlags, pfSelectionOnly);
      LObjPrinter := TMPHCanvasPrinter.Create(self, ACanvas, LSetFlags, AMargins, FPrintOptions.FHeaders);
      try
         Result := LObjPrinter.Pages;
         if APage > Result then raise EMPHexEditor.Create(ERR_INVALID_PAGE);
         if APage > 0 then
            if LObjPrinter.DrawOrCalc(False, APage) < 1 then raise EMPHexEditor.Create(ERR_PRINTING_FAILED);
      finally
         LObjPrinter.Free;
      end;
   finally
...
begin
   ...
   with Result do begin
      ...
      with LcnvMeta do
         try
            if FUseEditorFontForPrinting then Font.Assign(self.Font)
            else Font.Assign(self.FPrintFont);
...
var
   LmtfTemp: TMetaFile;
begin
   if Page < 1 then raise EMPHexEditor.Create(ERR_INVALID_PAGE);
   LmtfTemp := PrintPreview(Page);
   with LmtfTemp do
      try
         Printer.Canvas.StretchDraw(Rect(0, 0, Printer.PageWidth, Printer.PageHeight), LmtfTemp);
      finally
         Free;
      end;
end;
...
begin
   if OwnsClipBoard then begin
      if Flush then OleCheck(OleFlushClipboard)
      else OleSetClipboard(nil);
   end;
end;
...
begin
   inherited;
   if (not Handled) and (Assigned(FOffsetPopupMenu)) then begin
      // is mouse over offset col
      with MousePos do
         if ((X > -1) and (X < (ColWidths[0] + ColWidths[1]))) or ((Y > -1) and (Y < (RowHeights[0] + RowHeights[1]))) then begin
            // in fixed range
            if FOffsetPopupMenu.AutoPopup then begin
               Handled := True;
               SendCancelMode(nil);
               FOffsetPopupMenu.PopupComponent         := self;
               MousePos                                := ClientToScreen(MousePos);
               if InvalidPoint(MousePos) then MousePos := ClientToScreen(Point(0, 0));
...
procedure TMPHexEditorEx.Notification(AComponent: TComponent; Operation: TOperation);
begin
   inherited;
   if AComponent = FOffsetPopupMenu then
      if Operation = opRemove then OffsetPopupMenu := nil;
end;

function TMPHexEditorEx.CanCreateUndo(const aKind: TMPHUndoFlag; const aCount, aReplCount: Int64): boolean;
begin
   Result := inherited CanCreateUndo(aKind, aCount, aReplCount);
   if Result and (UndoStorage.UpdateCount > 0) then FModifiedNoUndo := True;
end;
...
begin
   Result       := '$';
   LBMSet       := False;
   for LIntLoop := low(TMPHBookMarks) to high(TMPHBookMarks) do begin
      AddBM(BookMark[LIntLoop]);
      if (not LBMSet) and (BookMark[LIntLoop].mPosition <> -1) then LBMSet := True;
   end;
   if not LBMSet then Result := '';
end;
...
begin
   if Action is TEditCut then Result        := CBCut
   else if Action is TEditCopy then Result  := CBCopy
   else if Action is TEditPaste then Result := CBPaste
   else if Action is TEditDelete then begin
      DeleteSelection(UNDO_CUTCB);
      Result := True;
   end
   else if Action is TEditUndo then Result := Undo
...
begin
   ...
   if Result then begin
      if Action is TEditAction then begin
         if Action is TEditCut then TEditAction(Action).Enabled := CanCut
         else if Action is TEditCopy then TEditAction(Action).Enabled := CanCopy
         else if Action is TEditPaste then TEditAction(Action).Enabled := CanPaste
         else if Action is TEditDelete then TEditAction(Action).Enabled := CanCut
         else if Action is TEditUndo then TEditUndo(Action).Enabled := CanUndo
         else if Action is TEditSelectAll then TEditSelectAll(Action).Enabled := DataSize > 0
         else Result := False;
      end
      else Result := inherited UpdateAction(Action);
   end;
end;
...
var
   ...
   begin
      Delete(Value, 1, 1);
      try
         for LIntLoop := low(TMPHBookMarks) to high(TMPHBookMarks) do ExtractBM(LIntLoop);
      except
...
begin
   ...
   try
      ...
      for LIntLoop          := low(TMPHBookMarks) to high(TMPHBookMarks) do BookMark[LIntLoop] := LRecBook;

      if Value <> '' then begin
         if Value[1] = '$' then NewSetBookmarks
         else
            try
               // check sum
               ...
               // calc check sum
               LIntCheck1   := 0;
               for LIntLoop := 1 to Length(Value) do LIntCheck1 := LIntCheck1 + Ord(Value[LIntLoop]);

               if LIntCheck1 <> LIntCheck then raise EMPHexEditor.Create(ERR_INVALID_BOOKFMT);
               ...
               while Value <> '' do begin
                  ...
                  if (LRecBook.mPosition <> LIntPos) or (LRecBook.mInCharField <> LBoolChars) then begin
...
begin
   with List do
      if index >= 0 then Result := Copy(Strings[index], Length(Names[index]) + 2, MaxInt)
      else Result               := '';
end;
{$ENDIF}
...
begin
   ...
   for LIntLoop := low(MPTH_PUBLIC_PROPS) to high(MPTH_PUBLIC_PROPS) do
      if AnsiCompareText(PropName, MPTH_PUBLIC_PROPS[LIntLoop]) = 0 then begin
         Result := True;
         Break;
      end;
   if Result and Assigned(FOnQueryPublicProperty) then FOnQueryPublicProperty(self, PropName, Result);
end;
         ...
         try
            for LIntCount := 0 to Pred(LIntCount) do
               with LPtrProps^[LIntCount]^ do
                  if PropType^^.Kind = tkClass then Recurse(GetObjectProp(Ref, name), Prefix + name + '.')
                  else if IsPropPublic(Prefix + name) then sl.Add(Prefix + name + '=' + string(GetPropValue(Ref, name{$IFDEF DELPHI7UP}, False{$ENDIF})));
   ...
   try
      ...
      with LStrData do
         try
            Text := Value;
            if Count > 0 then
               for LIntLoop := 0 to Pred(Count) do begin
                  LStrProp  := Names[LIntLoop];
                  if IsPropPublic(LStrProp) then begin
                     {$IFNDEF DELPHI7UP}
                     ...
                     repeat
                        ...
                        if LIntDot > 0 then begin
                           LObjProp := GetObjectProp(LObjProp, Copy(LStrProp, 1, LIntDot - 1));
                           System.Delete(LStrProp, 1, LIntDot);
                        end;
                     until LIntDot = 0;
                     if Assigned(LObjProp) then begin
                        // check type of data to avoid conversion errors
                        if TryStrToInt64(LStrVal, LIntVal64) then begin
                           if TryStrToInt(LStrVal, LIntVal) and (LIntVal = LIntVal64) then SetPropValue(LObjProp, LStrProp, LIntVal)
                           else SetPropValue(LObjProp, LStrProp, LIntVal64)
                        end
                        else SetPropValue(LObjProp, LStrProp, LStrVal);
                     end;
                  end;
               end;
         finally
...
begin
   // inherited;
   if FPaintUpdateCounter < 1 then inherited;
end;
...
begin
   ...
   with CheckMouseCoord(LptMouse.X, LptMouse.Y) do LIntPos := GetPosAtCursor(X, Y);
   if (LIntPos > -1) and (LIntPos < DataSize) then begin
...
procedure TMPHexEditorEx.PasteData(P: Pointer; const aCount: integer; const UndoDesc: string);
var
...
begin
   ...
   if SelCount = 0 then begin
      ...
      if (LIntPos mod BytesPerUnit) <> 0 then begin
         while (LIntPos mod BytesPerUnit) <> 0 do Dec(LIntPos);
         LgrcCoords := GetCursorAtPos(LIntPos, InCharField);
         with LgrcCoords do begin
            Col := X;
            Row := Y;
         end;
      end;
   end;
   if (SelCount = 0) and NoSizeChange then begin
      SelStart := LIntPos;
...
begin
   if not FBookmarksNoChange then inherited;
end;

function TMPHexEditorEx.UndoBeginUpdate(const StrUndoDesc: string = ''): integer;
begin
   if (UndoStorage.UpdateCount < 1) then
      try
         if (FCreateUndoOnUndoUpdate or (StrUndoDesc <> '')) then begin
...
begin
   ...
   if (Result < 1) then
      try
         if FCreateUndoOnUndoUpdate then begin
            if FModifiedNoUndo then FModifiedNoUndo := False
            else begin
...
procedure TMPHexEditorEx.WriteBuffer(const ABuffer; const AIndex, ACount: Int64);
begin
   inherited;
   FModified                                           := True;
   if UndoStorage.UpdateCount > 0 then FModifiedNoUndo := True;
end;
...
function TMPHDropTarget.DragEnter(const dataObj: IDataObject; grfKeyState: integer; pt: TPoint; var dwEffect: integer): HRESULT;
begin
   Result := FEditor.SupportsOLEData(dataObj, grfKeyState, pt, dwEffect, oleDrop);
...
function TMPHDropTarget.DragOver(grfKeyState: integer; pt: TPoint; var dwEffect: integer): HRESULT;
begin
   Result := FEditor.ModifyOLEDropEffect(grfKeyState, pt, dwEffect);
   if Result = S_OK then begin
      pt := FEditor.ScreenToClient(pt);
...
function TMPHDropTarget.Drop(const dataObj: IDataObject; grfKeyState: integer; pt: TPoint; var dwEffect: integer): HRESULT;
begin
   try
      Result := FEditor.SupportsOLEData(dataObj, grfKeyState, pt, dwEffect, oleDrop);
      if Result = S_OK then begin
         ...
         if Result = S_OK then
            try
               Result := FEditor.InsertOLEData(dataObj, grfKeyState, pt, dwEffect, oleDrop);
            except
               Result := E_FAIL;
...
begin
   FFormats := nil;
   if Succeeded(dataObject.EnumFormatEtc(DATADIR_GET, LifEnum)) then begin
      while LifEnum.Next(1, LRecFormat, nil) = S_OK do begin
...
begin
   ...
   if Length(FFormats) > 0 then
      for LIntLoop := 0 to Pred(Length(FFormats)) do
         if FFormats[LIntLoop].cfFormat = cfFormat then begin
            LBoolOK := True;
            Result  := FFormats[LIntLoop];
            Break;
         end;
   if not LBoolOK then FillChar(Result, sizeof(Result), #$FF);
end;
...
function TMPHEnumFormatETC.Next(celt: integer; out elt; pceltFetched: PLongint): HRESULT;
var
   LIntLoop: integer;
   LRecOut: packed array [0 .. MY_SUPPORTED_FORMATS - 1] of TFormatETC absolute elt;
begin
   LIntLoop := 0;
   while (LIntLoop < celt) and (FIndex < MY_SUPPORTED_FORMATS) do begin
      LRecOut[LIntLoop] := FFormats[FIndex];
      Inc(FIndex);
      Inc(LIntLoop);
   end;

   if pceltFetched <> nil then pceltFetched^ := LIntLoop;

   if LIntLoop = celt then Result := S_OK
   else Result                    := S_FALSE;
end;
...
begin
   if (celt < MY_SUPPORTED_FORMATS - FIndex) then begin
      FIndex := FIndex + celt;
      Result := S_OK;
   end
   else Result := S_FALSE;
...
begin
   case dwEffect and 7 of
      DROPEFFECT_NONE, DROPEFFECT_COPY, DROPEFFECT_MOVE: Result := DRAGDROP_S_USEDEFAULTCURSORS;
...
function TMPHDropSource.QueryContinueDrag(fEscapePressed: BOOL; grfKeyState: integer): HRESULT;
begin
...
constructor TMPHDataObject.Create(Data: Pointer; DataSize: integer; ScrapFileName: ShortString; TextAsHex, SwapNibbles: boolean);
begin
   inherited Create;
   FData          := nil;
   ...
   if Assigned(Data) and (DataSize > 0) then begin
      FDataSize := DataSize;
      FFileName := Format(STR_SCRAPFILE, [ChangeFileExt(ExtractFileName(ScrapFileName), ''), ExtractFileExt(ScrapFileName)]);
      GetMem(FData, DataSize);
      Move(Data^, FData^, FDataSize);
   end;
end;

constructor TMPHDataObject.CreateFromStream(Stream: TStream; Position, DataSize: integer; ScrapFileName: ShortString; TextAsHex, SwapNibbles: boolean);
begin
   inherited Create;
   FData          := nil;
   FHasDropEffect := False;
   FTextAsHex     := TextAsHex;
   FSwapNibbles   := SwapNibbles;
   if Assigned(Stream) and (DataSize > 0) then begin
      FDataSize := DataSize;
      FFileName := Format(STR_SCRAPFILE, [ChangeFileExt(ExtractFileName(ScrapFileName), ''), ExtractFileExt(ScrapFileName)]);
      GetMem(FData, DataSize);
...
begin
   if Assigned(FData) and (FDataSize > 0) then FreeMem(FData);
   FData     := nil;
   FDataSize := 0;
   inherited;
end;
...
function TMPHDataObject.DAdvise(const formatetc: TFormatETC; advf: integer; const advSink: IAdviseSink; out dwConnection: integer): HRESULT;
begin
...
function TMPHDataObject.EnumFormatEtc(dwDirection: integer; out EnumFormatEtc: IEnumFormatETC): HRESULT;
begin
   EnumFormatEtc := nil;
...
function TMPHDataObject.GetCanonicalFormatEtc(const formatetc: TFormatETC; out formatetcOut: TFormatETC): HRESULT;
begin
...
function TMPHDataObject.GetData(const formatetcIn: TFormatETC; out medium: TStgMedium): HRESULT;
var
...
begin
   ...
   if Result = S_OK then begin
      if formatetcIn.cfFormat = CF_MPHEXEDITOR then LIntDataSize := sizeof(TClipData) - 1 + FDataSize
      else if formatetcIn.cfFormat = CF_REGEDIT_HEXDATA then LIntDataSize := sizeof(TRegEditHexData) - 1 + FDataSize
      else if formatetcIn.cfFormat = CF_TEXT then begin
         if not FTextAsHex then LIntDataSize := Min(FDataSize, StrLen(PChar(FData))) + 1
         else LIntDataSize                   := (FDataSize * 2) + 1;
      end
      else if formatetcIn.cfFormat = CF_FILEDESCRIPTOR then LIntDataSize := sizeof(TFileGroupDescriptor)
      else LIntDataSize := FDataSize;
      medium.HGlobal                    := GlobalAlloc(GMEM_SHARE or GMEM_MOVEABLE or GMEM_ZEROINIT, LIntDataSize);
      if medium.HGlobal = 0 then Result := E_OUTOFMEMORY
         ...
         try
            try
               ...
               if formatetcIn.cfFormat = CF_TEXT then begin
                  if FTextAsHex then ConvertBinToHex(FData, PAnsiChar(LPtrLocal), FDataSize, FSwapNibbles)
                  else Move(FData^, LPtrLocal^, LIntDataSize - 1);
                  PChar(LPtrLocal)[LIntDataSize - 1] := #0;
               end
               else if formatetcIn.cfFormat = CF_MPHEXEDITOR then begin
                  LPtrLocal^.Signature := CLIP_SIG;
                  LPtrLocal^.Version   := CLIP_VER;
                  LPtrLocal^.Size      := FDataSize;
                  Move(FData^, LPtrLocal^.Data, FDataSize);
               end
               else if formatetcIn.cfFormat = CF_REGEDIT_HEXDATA then begin
                  Pointer(LPtrRegData) := Pointer(LPtrLocal);
                  LPtrRegData^.Size    := FDataSize;
                  Move(FData^, LPtrRegData^.Data, FDataSize);
               end
               else if formatetcIn.cfFormat = CF_FILEDESCRIPTOR then begin
                  with PFileGroupDescriptor(LPtrLocal)^ do begin
                     cItems := 1;
                     with fgd[0] do begin
                        ...
                        Move(FFileName[1], cFileName, Min(Length(FFileName), sizeof(cFileName) - 1));
                     end;
                  end;
               end else begin
...
function TMPHDataObject.GetDataHere(const formatetc: TFormatETC; out medium: TStgMedium): HRESULT;
begin
...
begin
   ...
   with formatetc do begin
      if dwAspect <> DVASPECT_CONTENT then Result := DV_E_DVASPECT
      else if not BOOL(tymed and TYMED_HGLOBAL)
      { // multiple tymeds may be queried (e.g. from explorer, wordpad...) } then Result := DV_E_TYMED
      else if (lindex <> -1) and ((cfFormat <> CF_FILECONTENTS) and (lindex <> 0)) then Result := DV_E_LINDEX
      else if (cfFormat = CF_MPHEXEDITOR) or (cfFormat = CF_TEXT) or (cfFormat = CF_FILEDESCRIPTOR) or (cfFormat = CF_FILECONTENTS) or (cfFormat = CF_REGEDIT_HEXDATA) then Result := S_OK;
   end;
end;

// check for dropeffect calls (dodragdrop not always return the real effect)

function TMPHDataObject.SetData(const formatetc: TFormatETC; var medium: TStgMedium; fRelease: BOOL): HRESULT;
var
   LPtrEffect: PDWORD;
begin
   Result := E_NOTIMPL;
   if ((formatetc.cfFormat = CF_PERFORMEDDROPEFFECT) or (formatetc.cfFormat = CF_LOGICALPERFORMEDDROPEFFECT)) and (medium.tymed = TYMED_HGLOBAL) then begin
      Result := S_OK;
      // check drop effect
   ...
   if fRelease then ReleaseStgMedium(medium);
end;
...
constructor TMPHCanvasPrinter.Create(AEditor: TMPHexEditorEx; ACanvas: TCanvas; AFlags: TMPHPrintFlags; AMargins: TRect; AHeaders: TMPHPrintHeaders);
begin
   inherited Create;
   FMargins    := AMargins;
   FCanvas     := ACanvas;
...
function TMPHCanvasPrinter.BuildHeader(const S: string; const Page: integer): string;
var
   LIntLoop: integer;
begin
   Result   := '';
   LIntLoop := 1;
   while LIntLoop <= Length(S) do begin
      if (S[LIntLoop] = '%') and (LIntLoop < Length(S)) then begin
         Inc(LIntLoop);
         case S[LIntLoop] of
            ...
            '>': begin
                  if not FEditor.UnicodeChars then Result := Result + MPHTranslationDesc[FEditor.Translation]
                  else begin
                     if not FEditor.UnicodeBigEndian then Result := Result + MPH_UC
                     else Result := Result + MPH_UC_BE
                  end;
               end;
            '<': begin
                  if not FEditor.UnicodeChars then Result := Result + MPHTranslationDescShort[FEditor.Translation]
                  else begin
                     if not FEditor.UnicodeBigEndian then Result := Result + MPH_UC_S
                     else Result := Result + MPH_UC_BE_S
                  end;
               end
            else Result := Result + '%' + S[LIntLoop];
            end;
      end
      else Result := Result + S[LIntLoop];
      Inc(LIntLoop);
...
function TMPHCanvasPrinter.DrawOrCalc(const JustCalc: boolean; const Page: integer): integer;

// return one line of data
   function GetOneLine(CurPosition, EndPosition: integer; const MinLen: integer): TTextWithAttr;
   ...
   // add spacer
      ...
      begin
         ...
         if UseDefAttr or (Length(Result.Attributes) = 1) then
            with Result.Attributes[Length(Result.Attributes) - 1] do begin
               Bold := False;

               Fore := FEditor.Font.Color;
               Back := FEditor.Colors.Background;
            end
         else Result.Attributes[Length(Result.Attributes) - 1] := Result.Attributes[Length(Result.Attributes) - 2]
      end;

   // get hex representation of data (or empty if > datasize)
      function GetByteHex(CurPosition, EndPosition: integer): string;
      begin
         if CurPosition > EndPosition then Result := '  '
         else begin
            if FEditor.HexLowerCase then Result := LowerCase(IntToRadixLen(FEditor.Data[CurPosition], 16, 2))
            else Result                         := UpperCase(IntToRadixLen(FEditor.Data[CurPosition], 16, 2));
            if FEditor.SwapNibbles and (Length(Result) = 2) then Result := Result[2] + Result[1];
         end;
      end;

   var
      LIntLoop, LIntLoopAttr: integer;
      LStrPart: string;
      LWChrText: widechar;
      lBold: boolean;
      lOdd: boolean;
   ...
   begin
      ...
      if LStrPart <> '' then begin
         LStrPart := StringOfChar(' ', MinLen - Length(LStrPart)) + LStrPart;
         if (not(pfUseBackgroundColor in FFlags)) or (pfMonochrome in FFlags) then LStrPart := LStrPart + ':';
         LStrPart := LStrPart + ' ';
      end;

      Result.Text := LStrPart;
      SetLength(Result.Attributes, Length(Result.Text));
      lBold        := (FEditor.Row - FEditor.FixedRows) = (CurPosition div FEditor.BytesPerRow);
      for LIntLoop := 1 to Length(Result.Text) do
         with Result.Attributes[LIntLoop - 1] do begin
            Bold := lBold;

            if (LIntLoop = Length(Result.Text)) or (not(pfUseBackgroundColor in FFlags)) then begin
               if (LIntLoop = Length(Result.Text)) and (pfUseBackgroundColor in FFlags) then Bold := False;
               Fore := FEditor.Font.Color;
               Back := FEditor.Colors.Background;
            end else begin
               if lBold then begin
                  if (pfUseBackgroundColor in FFlags) and not(pfMonochrome in FFlags) then Bold := False;
      ...
      if FEditor.InCharField then lBack := FEditor.Colors.Background
      else lBack                        := FEditor.Colors.ActiveFieldBackground;
      lOdd                              := True;
      for LIntLoop                      := 1 to FEditor.BytesPerRow do begin
         ...
         SetLength(Result.Attributes, Length(Result.Attributes) + Length(LStrPart));

         for LIntLoopAttr := LIntLoopAttr to Pred(Length(Result.Attributes)) do
            with Result.Attributes[LIntLoopAttr] do begin
               Bold := FEditor.IsSelected(CurPosition - 1 + LIntLoop);
               Fore := lFore;
               Back := lBack;
               if FEditor.ByteChanged[CurPosition - 1 + LIntLoop] then begin
         ...
         if LIntLoop < FEditor.BytesPerRow then begin

            if (FEditor.BytesPerBlock > 1) and ((LIntLoop mod FEditor.BytesPerBlock) = 0) then AddSpacer;

            if (LIntLoop mod FEditor.BytesPerColumn) = 0 then begin
               AddSpacer;
               lOdd := not lOdd;
               if lOdd then begin
                  lFore                             := FEditor.Colors.OddColumn;
      ...
      if not FEditor.UnicodeChars then begin
         for LIntLoop := 1 to FEditor.BytesPerRow do begin
            if (CurPosition + LIntLoop - 1) > EndPosition then Result.Text := Result.Text + ' '
            else Result.Text := Result.Text + FEditor.TranslateToAnsiChar(FEditor.Data[CurPosition + LIntLoop - 1]);

            SetLength(Result.Attributes, Length(Result.Attributes) + 1);

            with Result.Attributes[Pred(Length(Result.Attributes))] do begin
               Bold := FEditor.IsSelected(CurPosition - 1 + LIntLoop);
               if FEditor.ByteChanged[CurPosition - 1 + LIntLoop] then begin
                  Fore := FEditor.Colors.ChangedText;
                  Back := FEditor.Colors.ChangedBackGround;
                  ...
                  Fore                                 := FEditor.Font.Color;
                  if not FEditor.InCharField then Back := FEditor.Colors.Background
                  else Back                            := FEditor.Colors.ActiveFieldBackground;
               end;

            end;

            if LIntLoop < FEditor.BytesPerRow then begin
               if (FEditor.BytesPerBlock > 1) and FEditor.SeparateBlocksInCharField and ((LIntLoop mod FEditor.BytesPerBlock) = 0) then AddSpacer;

               if (FEditor.UsedRulerBytesPerUnit <> 1) and ((LIntLoop mod FEditor.UsedRulerBytesPerUnit) = 0) then AddSpacer;
            end;
         end;
      end
      else
         for LIntLoop := 0 to Pred(FEditor.BytesPerRow) div 2 do begin
            if (CurPosition + (LIntLoop * 2)) > EndPosition then begin
               Result.Text := Result.Text + ' ';
            end else begin
               FEditor.ReadBuffer(LWChrText, CurPosition + (LIntLoop * 2), 2);
               if FEditor.UnicodeBigEndian then SwapWideChar(LWChrText);
               if (LWChrText < #256) and (AnsiChar(LWChrText) in FEditor.MaskedChars) then begin
            ...
            with Result.Attributes[Pred(Length(Result.Attributes))] do begin
               ...
               if FEditor.ByteChanged[CurPosition + (LIntLoop * 2)] or FEditor.ByteChanged[(CurPosition + (LIntLoop * 2)) + 1] then begin
                  Fore := FEditor.Colors.ChangedText;
                  Back := FEditor.Colors.ChangedBackGround;
               end else begin
                  Fore                                 := FEditor.Font.Color;
            ...
            if LIntLoop < FEditor.BytesPerRow then begin
               if (FEditor.BytesPerBlock > 1) and FEditor.SeparateBlocksInCharField and (((LIntLoop + 1) mod (FEditor.BytesPerBlock div 2)) = 0) then AddSpacer;

               if (FEditor.UsedRulerBytesPerUnit <> 2) and (((LIntLoop * 2) mod FEditor.UsedRulerBytesPerUnit) = 0) then AddSpacer;
            end;
         end;
   end;
...
// return ruler line
   ...
   begin
      ...
      for LIntLoop   := 1 to Length(FEditor.FRulerString) do begin
         ...
         if LIntLoop <> Length(FEditor.FRulerString) then begin
            if (FEditor.BytesPerBlock > 1) and ((LIntLoop mod (FEditor.BytesPerBlock * 2)) = 0) then AddSpacer;
      ...
      for LIntLoop   := 1 to Length(FEditor.FRulerCharString) do begin
         ...
         if LIntLoop <> Length(FEditor.FRulerCharString) then begin
            if not FEditor.UnicodeChars then begin
               if (FEditor.BytesPerBlock > 1) and FEditor.SeparateBlocksInCharField and ((LIntLoop mod FEditor.BytesPerBlock) = 0) then AddSpacer;

               if (FEditor.UsedRulerBytesPerUnit <> 1) and ((LIntLoop mod FEditor.UsedRulerBytesPerUnit) = 0) then AddSpacer;
            end else begin
               if (FEditor.BytesPerBlock > 1) and FEditor.SeparateBlocksInCharField and ((LIntLoop mod (FEditor.BytesPerBlock div 2)) = 0) then AddSpacer;

               if (FEditor.UsedRulerBytesPerUnit <> 2) and ((((LIntLoop - 1) * 2) mod FEditor.UsedRulerBytesPerUnit) = 0) then AddSpacer;
            end;
         end;
      end;
   end;
   ...
   begin
      ...
      if LIntPipe > 0 then begin
         ...
         if LIntPipe > 0 then begin
            ...
            Delete(StrText, 1, LIntPipe);
            if StrText <> '' then LStrRight := StrText;
         end
         else LStrCenter := StrText;
      end
      else LStrLeft := StrText;
      ...
      try
         ...
         if LStrRight <> '' then begin
            SetTextAlign(FCanvas.Handle, TA_TOP or TA_RIGHT);
            ExtTextOut(FCanvas.Handle, RightPos, Y, ETO_CLIPPED, @LIntRect, PChar(LStrRight), Length(LStrRight), nil);
         end;
         if LStrCenter <> '' then begin
            SetTextAlign(FCanvas.Handle, TA_TOP or TA_CENTER);
            ExtTextOut(FCanvas.Handle, LeftPos + ((RightPos - LeftPos) div 2), Y, ETO_CLIPPED, @LIntRect, PChar(LStrCenter), Length(LStrCenter), nil);
         end;
         if LStrLeft <> '' then begin
            SetTextAlign(FCanvas.Handle, TA_TOP or TA_LEFT);
            ExtTextOut(FCanvas.Handle, LeftPos, Y, ETO_CLIPPED, @LIntRect, PChar(LStrLeft), Length(LStrLeft), nil);
         end;
      finally
...
var
   ...
   LIntWidth, LIntHeight, LIntDataPos, LIntLeft, LIntY, LIntMaxY, LIntDataEnd, LIntDataStart: integer;
   LclrFSave: TColor;
   LclrBSave: TColor;
   LfstSave: TFontStyles;
   LIntLoop: integer;
   LIntMinWidth, LIntOldFontSize: integer;
   LRectOut: TRect;
   LRecSize: TSize;
begin
   ...
   Result           := -1;
   if (not Assigned(FEditor)) or (FEditor.DataSize < 1) then Exit;

   LIntMinWidth := Length(FEditor.GetOffsetString(FEditor.DataSize));

   if (not JustCalc) and (FLinesPerPage < 1) then Exit;

   if (pfSelectionOnly in FFlags) and (FEditor.SelCount > 0) then begin
      LIntDataEnd   := FEditor.SelEnd;
      LIntDataStart := FEditor.SelStart;
      if LIntDataStart > LIntDataEnd then begin
         LIntDataStart := FEditor.SelEnd;
         LIntDataEnd   := FEditor.SelStart;
      end;
   end else begin
      if (pfCurrentViewOnly in FFlags) then begin
   ...
   if not(JustCalc) then LIntDataStart := LIntDataStart + ((Page - 1) * (FLinesPerPage * FEditor.BytesPerRow));

   if LIntDataStart > LIntDataEnd then Exit;
   ...
   try
      if (pfMonochrome in FFlags) or (not(pfUseBackgroundColor in FFlags)) then FCanvas.Brush.Color := clWhite
      else FCanvas.Brush.Color := FEditor.Colors.Background;
      FCanvas.Brush.Style                                 := bsSolid;
      if (pfMonochrome in FFlags) then FCanvas.Font.Color := clBlack
      else FCanvas.Font.Color                             := FEditor.Font.Color;
      if not JustCalc then FCanvas.FillRect(FMargins);
      LIntWidth := FCanvas.TextWidth(LRecTextAttr.Text);
      while (LIntWidth > (FMargins.Right - FMargins.Left)) and (FCanvas.Font.Size > 1) do begin
      ...
      FPrintHeaders[0] := BuildHeader(FHeaders[0], Page);
      FPrintHeaders[1] := BuildHeader(FHeaders[1], Page);
      if FPrintHeaders[0] <> '' then begin
         if not JustCalc then begin
      ...
      if FPrintHeaders[1] <> '' then LIntMaxY := LIntMaxY - LIntHeight;

      if (pfIncludeRuler in FFlags) and FEditor.ShowRuler then begin
         if not JustCalc then begin
            ...
            for LIntLoop := 1 to Length(LRecTextAttr.Text) do begin
               if not(pfMonochrome in FFlags) then begin
                  FCanvas.Font.Color := LRecTextAttr.Attributes[LIntLoop - 1].Fore;
                  if pfUseBackgroundColor in FFlags then FCanvas.Brush.Color := LRecTextAttr.Attributes[LIntLoop - 1].Back
                  else if LRecTextAttr.Attributes[LIntLoop - 1].Fore = clWhite then FCanvas.Font.Color := clBlack;
               end;

               if FFlags * [pfSelectionBold, pfSelectionOnly] = [pfSelectionBold] then begin
                  if FFlags * [pfMonochrome, pfUseBackgroundColor] = [pfUseBackgroundColor] then begin
                     FCanvas.Font.Style := [];
                     if LRecTextAttr.Attributes[LIntLoop - 1].Bold then begin
                        FCanvas.Font.Color  := ColorToRGB(FCanvas.Font.Color) xor $FFFFFF;
                        FCanvas.Brush.Color := ColorToRGB(FCanvas.Brush.Color) xor $FFFFFF;
                     end;

                  end else begin
                     if LRecTextAttr.Attributes[LIntLoop - 1].Bold then FCanvas.Font.Style := [fsBold]
                     else FCanvas.Font.Style := [];
                  end;
               end;

               LRectOut := Rect(LIntLeft, LIntY, LIntLeft + FCanvas.TextWidth('w'), LIntY + LIntHeight);
               if (not(pfUseBackgroundColor in FFlags)) or (pfMonochrome in FFlags) then LRectOut.Bottom := LIntY + (LIntHeight * 3 div 2);
               ExtTextOutW(FCanvas.Handle, LIntLeft, LIntY, ETO_CLIPPED or ETO_OPAQUE, @LRectOut, @LRecTextAttr.Text[LIntLoop], 1, nil);
               if (not(pfUseBackgroundColor in FFlags)) or (pfMonochrome in FFlags) then begin
         ...
         if (not(pfUseBackgroundColor in FFlags)) or (pfMonochrome in FFlags) then LIntY := LIntY + (LIntHeight * 3 div 2)
         else LIntY := LIntY + LIntHeight;
      end;

      while (LIntHeight + LIntY) <= LIntMaxY do begin
         if not JustCalc then begin
            LclrFSave    := FCanvas.Font.Color;
            LclrBSave    := FCanvas.Brush.Color;
            LfstSave     := FCanvas.Font.Style;
            ...
            for LIntLoop := 1 to Length(LRecTextAttr.Text) do begin
               ...
               if FEditor.UnicodeChars then
                  while (FCanvas.Font.Size > 1) and GetTextExtentPoint32W(FCanvas.Handle, @LRecTextAttr.Text[LIntLoop], 1, LRecSize) and (LRecSize.cx > (LRectOut.Right - LRectOut.Left)) do FCanvas.Font.Size := FCanvas.Font.Size - 1;
               ExtTextOutW(FCanvas.Handle, LIntLeft, LIntY, ETO_CLIPPED or ETO_OPAQUE, @LRectOut, @LRecTextAttr.Text[LIntLoop], 1, nil);
               if FEditor.UnicodeChars then FCanvas.Font.Size := LIntOldFontSize;
               LIntLeft := LRectOut.Right;
            end;
            FCanvas.Font.Color  := LclrFSave;
            FCanvas.Brush.Color := LclrBSave;
         ...
         if LIntDataPos > LIntDataEnd then begin
            Break;
         end;
         if not JustCalc then LRecTextAttr := GetOneLine(LIntDataPos, LIntDataEnd, LIntMinWidth);
         LIntY                             := LIntY + LIntHeight;
      end;

      if FPrintHeaders[1] <> '' then
         if not JustCalc then begin
            DrawHeader(FMargins.Left, FMargins.Bottom - LIntHeight, FMargins.Right, FPrintHeaders[1]);
...
begin
   ...
   if pfSelectionOnly in FFlags then LIntSize := Abs(FEditor.SelStart - FEditor.SelEnd)
   else if pfCurrentViewOnly in FFlags then begin
      LIntSize := Abs(FEditor.DisplayEnd - FEditor.DisplayStart);
   end
   else LIntSize := FEditor.DataSize;

   while (LIntSize mod FEditor.BytesPerRow) <> 0 do Inc(LIntSize);
   LIntSize := LIntSize div FEditor.BytesPerRow;
   while (LIntSize mod FLinesPerPage) <> 0 do Inc(LIntSize);
...
begin
   ...
   if Source is TMPHPrintOptions then
      with TMPHPrintOptions(Source) do begin
...
begin
   case index of
      ...
      else Result := FMargins.Bottom;
      end;
end;

// set haeder/footer

procedure TMPHPrintOptions.SetHeader(const index: integer; const Value: string);
begin
...
begin
   with LlbxFormats do LcbxTextAsHex.Enabled := (ItemIndex > -1) and (TClipFormat(Items.Objects[ItemIndex]) in [CF_TEXT, CF_OEMTEXT])
end;

procedure MPHSetHexClipboardData(Data: Pointer; DataSize: integer; ScrapFileName: ShortString; TextAsHex, SwapNibbles: boolean);
begin
   OleCheck(OleSetClipboard(TMPHDataObject.Create(Data, DataSize, ScrapFileName, TextAsHex, SwapNibbles)));
end;
