...
// File:      FormUnitHexEditorMain.pas
// Compiler:  Delphi
// *****************************************************************************
// Dependencies:
// *****************************************************************************
...
type
   TFormHexEditorMain = class(TForm)
      ...
      procedure MPHexEditorEx1Progress(Sender: TObject; const ProgressType: TMPHProgressKind; const aName: TFileName; const Percent: Byte; var Cancel: Boolean);
...
var
   FormHexEditorMain: TFormHexEditorMain;

implementation

uses
   FormUnitConvert,
   FormUnitCustomTranslation,
   dcrUIDlgPrintHex,
   FormUnitInputNumber,
   FormUnitOptions;
...
const
   ...
   STR_ERR_BPU_FINDTEXT    = 'Size of data to search for must be a multiple ' + 'of Bytes per unit';
   STR_ERR_BPU_REPLACETEXT = 'Size of replacement data must be a multiple ' + 'of Bytes per unit';

procedure TFormHexEditorMain.acFileOpenExecute(Sender: TObject);
begin
   if CheckChanges then begin
      SaveBookmarks;
      with OpenDialog1 do
         if Execute then
            with MPHexEditorEx1 do begin
               try
...
procedure TFormHexEditorMain.acFileNewExecute(Sender: TObject);
begin
   if CheckChanges then begin
...
function TFormHexEditorMain.CheckChanges: Boolean;
begin
   with MPHexEditorEx1 do begin
      if Modified then
...
function TFormHexEditorMain.SaveFile(const ForceNewName: Boolean): Boolean;
begin
   Result := True;
   with MPHexEditorEx1 do
      try
         if (not Modified) or (not HasFile) or (ReadOnlyFile) or ForceNewName then begin
            with SaveDialog1 do
...
procedure TFormHexEditorMain.FormCreate(Sender: TObject);
var
...
begin
   ...
   for LEnumTrans := high(TMPHTranslationKind) downto low(TMPHTranslationKind) do begin
      FTranslationActions[LEnumTrans] := TAction.Create(self);
      with FTranslationActions[LEnumTrans] do begin
   ...
   // check command line
   if (ParamCount in [1, 2]) and FileExists(ParamStr(1)) then begin
      with MPHexEditorEx1 do begin
         try
      ...
      if ParamStr(2) = '/p' then begin
         MPHexEditorEx1.PrintOptions.Flags := MPHexEditorEx1.PrintOptions.Flags - [pfSelectionOnly, pfCurrentViewOnly];
         HexPrintPreview(MPHexEditorEx1);
...
procedure TFormHexEditorMain.AppIdle(Sender: TObject; var Done: Boolean);
var
   LEnumTrans: TMPHTranslationKind;
begin
   ...
   with MPHexEditorEx1 do begin
      ...
      with StatusBar1 do begin
         ...
         // selection
         if SelCount > 0 then Panels[2].Text := Format(STR_SB_SEL, [Trim(GetAnyOffsetString(Min(SelStart, SelEnd))), Trim(GetAnyOffsetString(Max(SelStart, SelEnd)))])
         else Panels[2].Text := '';
         // changed
         ...
         // translation / unicode
         if not UnicodeChars then Panels[6].Text := MPHTranslationDescShort[Translation]
         else begin
            if UnicodeBigEndian then Panels[6].Text := MPH_UC_BE_S
            else Panels[6].Text := MPH_UC_S
         end;
      end;
      ...
      with acEditConvert do begin
         Enabled := (DataSize > 0) and (not ReadOnlyView);
         if SelCount > 0 then Caption := STR_MENU_EDIT_CONVERT_SEL
         else Caption := STR_MENU_EDIT_CONVERT_DATA
      end;

      // - view
      // dynamic translation actions
      for LEnumTrans := low(TMPHTranslationKind) to high(TMPHTranslationKind) do
         with FTranslationActions[LEnumTrans] do Checked := Integer(Translation) = Tag;
      ...
      if AutoBytesPerRow then acViewBytesPerRow.Caption := Format(STR_MENU_VIEW_BPR_AUTO, [BytesPerRow])
      else acViewBytesPerRow.Caption := Format(STR_MENU_VIEW_BPR, [BytesPerRow]);
      ...
      if not acViewUnicode.Checked then acViewUnicode.Enabled := ((BytesPerRow mod 2) = 0) and ((BytesPerColumn mod 2) = 0)
      else acViewUnicode.Enabled := True;
      acViewUnicodeBE.Checked := UnicodeBigEndian;
      acViewUnicodeBE.Enabled := UnicodeChars;
   end;
end;

procedure TFormHexEditorMain.MPHexEditorEx1Progress(Sender: TObject; const ProgressType: TMPHProgressKind; const aName: TFileName; const Percent: Byte; var Cancel: Boolean);
begin
...
procedure TFormHexEditorMain.StatusBar1DrawPanel(StatusBar: TStatusBar; Panel: TStatusPanel; const Rect: TRect);
var
...
begin
   if Panel.Index = 7 then begin
      ...
      if LIntProgress > -1 then begin
         if LIntProgress = 100 then Panel.Text := '-' // progressing finished
         else begin
            ...
            with StatusBar.Canvas do begin
               ...
               with LRctTemp do Right := Left + Round((Right - Left) / 100 * LIntProgress);
...
procedure TFormHexEditorMain.acFileSaveExecute(Sender: TObject);
begin
   SaveFile(False);
end;

procedure TFormHexEditorMain.acFileSaveAsExecute(Sender: TObject);
begin
   SaveFile(True);
end;

procedure TFormHexEditorMain.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
...
procedure TFormHexEditorMain.acFileExitExecute(Sender: TObject);
begin
   Close;
end;

procedure TFormHexEditorMain.acEditUndoExecute(Sender: TObject);
begin
   MPHexEditorEx1.Undo;
end;

procedure TFormHexEditorMain.acEditRedoExecute(Sender: TObject);
begin
   MPHexEditorEx1.Redo;
end;

procedure TFormHexEditorMain.acEditCopyExecute(Sender: TObject);
begin
   MPHexEditorEx1.CBCopy;
end;

procedure TFormHexEditorMain.acEditCutExecute(Sender: TObject);
begin
   MPHexEditorEx1.CBCut;
end;

procedure TFormHexEditorMain.acEditPasteExecute(Sender: TObject);
begin
   MPHexEditorEx1.CBPaste;
end;

procedure TFormHexEditorMain.acEditSelectAllExecute(Sender: TObject);
begin
   MPHexEditorEx1.SelectAll;
end;

procedure TFormHexEditorMain.acEditInsertNibbleExecute(Sender: TObject);
begin
   with MPHexEditorEx1 do InsertNibble(GetCursorPos, InCharField or ((Col mod 2) = 0));
end;

procedure TFormHexEditorMain.acEditDeleteNibbleExecute(Sender: TObject);
begin
   with MPHexEditorEx1 do DeleteNibble(GetCursorPos, InCharField or ((Col mod 2) = 0));
end;

procedure TFormHexEditorMain.acEditFindExecute(Sender: TObject);
var
   LIntSize: Integer;
begin
   if FindGetOptions(FFindRec) then begin
      with FFindRec do begin
         if not BoolFindText then begin
            // convert hex data to string
            SetLength(StrData, Length(StrText));
            UniqueString(StrData);
            ConvertHexToBin(PAnsiChar(StrText), PAnsiChar(StrData), Length(StrText), MPHexEditorEx1.SwapNibbles, LIntSize);
...
procedure TFormHexEditorMain.acEditFindNextExecute(Sender: TObject);
var
   LIntFound, LIntPos: Integer;
   LStrFind: string;
begin
   with MPHexEditorEx1, FFindRec do
      if StrData <> '' then begin
         LIntPos := Max(0, GetCursorPos);
         if (Length(StrData) = 1) and ((SelCount) = 1) then Inc(LIntPos);
         if LIntPos >= DataSize then LIntFound := -1
         else begin
            LStrFind := PrepareFindReplaceData(StrData, BoolIgnoreCase, BoolFindText);
            if (Length(LStrFind) mod BytesPerUnit) <> 0 then begin
               MessageDlg(STR_ERR_BPU_FINDTEXT, mtError, [mbOK], 0);
               Exit;
            end;
            LIntFound := Find(PAnsiChar(LStrFind), Length(LStrFind), LIntPos, DataSize - 1, BoolIgnoreCase);
         end;
         if LIntFound = -1 then ShowMessage(STR_NOT_FOUND)
         else begin
...
procedure TFormHexEditorMain.acEditConvertExecute(Sender: TObject);
var
   LEnumFrom, LEnumTo: TMPHTranslationKind;
   LIntStart, LIntEnd: Integer;
begin
   with MPHexEditorEx1 do begin
      LEnumFrom := Translation;
      LEnumTo := Translation;
      if SelectConvertTranslation(LEnumFrom, LEnumTo) and (LEnumFrom <> LEnumTo) then begin
         ...
         if SelCount = 0 then Translation := LEnumTo;
      end;
   end;
end;

procedure TFormHexEditorMain.acViewTranslationEditCustomExecute(Sender: TObject);
begin
   with MPHexEditorEx1 do
      if EditCustomTranslation and (Translation = tkCustom) then Repaint;
end;

procedure TFormHexEditorMain.TranslationExecute(Sender: TObject);
begin
   with TAction(Sender) do MPHexEditorEx1.Translation := TMPHTranslationKind(Tag);
end;

procedure TFormHexEditorMain.acEditReplaceExecute(Sender: TObject);
var
   LIntSize, LIntPos, LIntDone, LIntPos2: Integer;

   LStrFind, LStrReplace: string;
begin
   if ReplaceGetOptions(FReplaceRec) then
      with MPHexEditorEx1, FReplaceRec do
         try
            if not BoolFindText then begin
               ...
               ConvertHexToBin(PAnsiChar(StrTextToFind), PAnsiChar(StrDataToFind), Length(StrTextToFind), SwapNibbles, LIntSize);
               SetLength(StrDataToFind, LIntSize);
            end;
            UniqueString(StrDataToReplace);
            if (not BoolReplaceText) and (StrTextToReplace <> '') then begin
               // convert hex data to string
               SetLength(StrDataToReplace, Length(StrTextToReplace));
               ConvertHexToBin(PAnsiChar(StrTextToReplace), PAnsiChar(StrDataToReplace), Length(StrTextToReplace), SwapNibbles, LIntSize);
            ...
            if BoolReplaceAll then LIntPos := 0
            else LIntPos := Max(0, GetCursorPos);
            ...
            if (Length(LStrReplace) mod BytesPerUnit) <> 0 then begin
               MessageDlg(STR_ERR_BPU_REPLACETEXT, mtError, [mbOK], 0);
               Exit;
            end;

            repeat
               LIntPos2 := Find(PAnsiChar(LStrFind), Length(LStrFind), LIntPos, DataSize - 1, BoolIgnoreCase);
               if LIntPos2 = -1 then Break;
               Inc(LIntDone);
               SelStart := LIntPos2;
               SelEnd := LIntPos2 + Length(LStrFind) - 1;
               if LStrReplace <> '' then ReplaceSelection(PAnsiChar(LStrReplace), Length(LStrReplace), '', False)
            ...
            if LIntDone = 0 then raise Exception.Create(STR_NOT_FOUND)
            else if BoolReplaceAll then ShowMessage(Format(STR_NUM_REPLACED, [LIntDone]));
         finally
            StatusBar1.Panels[7].Text := '-';
         end;
end;

procedure TFormHexEditorMain.acFilePrintSetupExecute(Sender: TObject);
begin
   PrinterSetupDialog1.Execute;
end;

procedure TFormHexEditorMain.acFilePrintExecute(Sender: TObject);
begin
   // PrintPreview(MPHexEditorEx1, acFilePrintSetup, False);
   HexPrintPreview(MPHexEditorEx1);
end;

procedure TFormHexEditorMain.acViewOffsetEditFormatExecute(Sender: TObject);
var
   LStrPrefix: string;
begin
   with MPHexEditorEx1 do begin
      LStrPrefix := OffsetFormat;
      if InputQuery(STR_QT_OFFSETPREFIX, STR_Q_OFFSETPREFIX, LStrPrefix) then OffsetFormat := LStrPrefix;
   end;
end;

procedure TFormHexEditorMain.acViewOffsetHexExecute(Sender: TObject);
begin
   MPHexEditorEx1.OffsetFormat := MPHOffsetHex;
   MPHexEditorEx1.RulerNumberBase := 16;
end;

procedure TFormHexEditorMain.acViewOffsetDecExecute(Sender: TObject);
begin
   MPHexEditorEx1.OffsetFormat := MPHOffsetDec;
   MPHexEditorEx1.RulerNumberBase := 10;
end;

procedure TFormHexEditorMain.acViewOffsetOctExecute(Sender: TObject);
begin
   MPHexEditorEx1.OffsetFormat := MPHOffsetOct;
   MPHexEditorEx1.RulerNumberBase := 8;
end;

procedure TFormHexEditorMain.LoadBookmarks;
begin
   with MPHexEditorEx1 do
      if HasFile then BookMarksAsString := FIni.ReadString(STR_INI_BOOKMARKS, FileName, '')
end;

procedure TFormHexEditorMain.SaveBookmarks;
begin
   with MPHexEditorEx1 do
      if HasFile then begin
         if BookMarksAsString = '' then FIni.DeleteKey(STR_INI_BOOKMARKS, FileName)
         else FIni.WriteString(STR_INI_BOOKMARKS, FileName, BookMarksAsString)
      end;
end;

procedure TFormHexEditorMain.acViewBytesPerRowExecute(Sender: TObject);
var
   LIntVal: Integer;
begin
   with MPHexEditorEx1 do begin
      LIntVal := BytesPerRow;
      if InputNumber(STR_INP_BPR, LIntVal, 0, 255) then begin
         if LIntVal = 0 then AutoBytesPerRow := True
         else begin
...
procedure TFormHexEditorMain.acViewBytesPerColumnExecute(Sender: TObject);
var
   LIntVal: Integer;
begin
   with MPHexEditorEx1 do begin
      LIntVal := BytesPerColumn;
      if InputNumber(STR_INP_BPC, LIntVal, 1, 255) then BytesPerColumn := LIntVal
   end;
end;

procedure TFormHexEditorMain.acEditGotoExecute(Sender: TObject);
var
   LChrRel: Char;
   LIntPos: Integer;
   LStrTemp: string;
begin
   with MPHexEditorEx1 do
      if InputQuery(STR_QT_GOTO, STR_Q_GOTO, FGoto) and (FGoto <> '') then begin
         LChrRel := #0;
         LStrTemp := FGoto;
         if LStrTemp[1] in ['-', '+', '!'] then begin
            LChrRel := LStrTemp[1];
            Delete(LStrTemp, 1, 1);
         end;
         ...
         case LChrRel of
            ...
            else Seek(LIntPos, soFromBeginning);
            end;
      end;
end;

procedure TFormHexEditorMain.FormDestroy(Sender: TObject);
begin
   FIni.UpdateFile;
   FreeAndNil(FIni);
end;

procedure TFormHexEditorMain.LoadEditorProps;
var
   LsrlData: TStrings;
begin
...
procedure TFormHexEditorMain.SaveEditorProps;
var
   LsrlData: TStrings;
   LIntPos: Integer;
begin
   ...
   try
      ...
      with LsrlData do begin
         Text := MPHexEditorEx1.PropertiesAsString;
         if Count > 0 then
            for LIntPos := 0 to Pred(Count) do FIni.WriteString(STR_INI_EDITOR, Names[LIntPos], Values[Names[LIntPos]]);
      end;
   finally
      FreeAndNil(LsrlData);
   end;
...
procedure TFormHexEditorMain.acViewReadOnlyViewExecute(Sender: TObject);
begin
   with MPHexEditorEx1 do ReadOnlyView := not ReadOnlyView;
end;

procedure TFormHexEditorMain.acViewOptionsExecute(Sender: TObject);
var
   LStrOptions: string;
begin
   with MPHexEditorEx1 do begin
      LStrOptions := PropertiesAsString;
      if EditEditorOptions(LStrOptions) then PropertiesAsString := LStrOptions;
   end;
end;

procedure TFormHexEditorMain.acViewBytesPerUnitExecute(Sender: TObject);
var
   LIntVal: Integer;
begin
   with MPHexEditorEx1 do begin
      LIntVal := BytesPerUnit;
      if InputNumber(STR_INP_BPU, LIntVal, 1, 8) then BytesPerUnit := LIntVal
   end;
end;

procedure TFormHexEditorMain.AboutTMPHexEditor1Click(Sender: TObject);
begin
   MessageDlg('TMPHexEditor version:'#13#10 + MPHexEditorEx1.Version, mtInformation, [mbOK], 0);
end;

procedure TFormHexEditorMain.acViewUnicodeExecute(Sender: TObject);
begin
   with MPHexEditorEx1 do UnicodeChars := not UnicodeChars;
end;

procedure TFormHexEditorMain.acViewUnicodeBEExecute(Sender: TObject);
begin
   with MPHexEditorEx1 do UnicodeBigEndian := not UnicodeBigEndian;
end;

procedure TFormHexEditorMain.acViewBytesPerBlockExecute(Sender: TObject);
var
   LIntVal: Integer;
begin
   with MPHexEditorEx1 do begin
      LIntVal := BytesPerBlock;
      if InputNumber(STR_INP_BPB, LIntVal, -1, BytesPerRow) then BytesPerBlock := LIntVal
   end;
end;
