...
type
   TDialogOptions = class(TForm)
      ...
      procedure ValueListEditor1Validate(Sender: TObject; ACol, ARow: Integer; const KeyName, KeyValue: string);
      procedure ValueListEditor1KeyPress(Sender: TObject; var Key: Char);
      procedure ValueListEditor1StringsChange(Sender: TObject);
      procedure HexerQueryPublicProperty(Sender: TObject; const PropertyName: string; var IsPublic: Boolean);
   private
...
function EditEditorOptions(var Options: string): Boolean;
begin
   with TDialogOptions.Create(Application) do
      try
         Hexer.PropertiesAsString := Options;
         SetProperties;
         Result := ShowModal = mrOK;
         if Result then Options := Hexer.PropertiesAsString;
      finally
         Free;
      end;
end;
...
const
   ...
   STR_BOOL: array [Boolean] of string = ('No', 'Yes');
   {$R *.dfm}

procedure TDialogOptions.FormCreate(Sender: TObject);
var
   LIntLoop: Integer;
   LStrData: string;
...
begin
   ...
   LStrData := '';
   for LIntLoop := 0 to 255 do LStrData := LStrData + Char(LIntLoop);
...
procedure TDialogOptions.cbColorsSelect(Sender: TObject);
begin
   Label2.Enabled := True;
   Label3.Enabled := True;
   cbFore.Enabled := True;
...
procedure TDialogOptions.cbForeSelect(Sender: TObject);
begin
   with cbColors do
      case ItemIndex of
         0: // Offset
...
procedure TDialogOptions.btnFontClick(Sender: TObject);
var
   LfntTemp: TFont;
begin
   ...
   try
      ...
      with FontDialog1 do begin
         ...
         if Execute then Hexer.Font.Assign(Font)
         else Hexer.Font.Assign(LfntTemp);
      end;
   finally
      FreeAndNil(LfntTemp);
   end;
...
procedure TDialogOptions.FontDialog1Apply(Sender: TObject; Wnd: HWND);
begin
   Hexer.Font.Assign(FontDialog1.Font);
end;

procedure TDialogOptions.SetProperties;
   ...
   begin
      with ValueListEditor1 do begin
         ...
         with ItemProps[LIntRow] do begin
...
procedure TDialogOptions.ValueListEditor1Validate(Sender: TObject; ACol, ARow: Integer; const KeyName, KeyValue: string);
begin
   with Hexer do
      case ARow of
         ...
         11: MaskChar := AnsiString((KeyValue + #0))[1];
...
procedure TDialogOptions.ValueListEditor1KeyPress(Sender: TObject; var Key: Char);
begin
   if Key = #13 then
      with ValueListEditor1 do ValueListEditor1Validate(ValueListEditor1, Col, Row, Keys[Row], Values[Keys[Row]]);
end;

procedure TDialogOptions.ValueListEditor1StringsChange(Sender: TObject);
begin
   with ValueListEditor1 do
      if Values[Keys[Row]] <> '' then ValueListEditor1Validate(ValueListEditor1, Col, Row, Keys[Row], Values[Keys[Row]]);

end;

procedure TDialogOptions.HexerQueryPublicProperty(Sender: TObject; const PropertyName: string; var IsPublic: Boolean);
const
