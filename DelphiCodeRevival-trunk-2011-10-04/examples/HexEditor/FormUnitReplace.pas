...
type
   ...
   TReplaceRec = record
      ...
      StrTextToReplace, StrDataToFind, // real data to find
      StrDataToReplace: string;        // real data to replace with
      BoolIgnoreCase,                  // ignore upper/lower case
      BoolFindText, BoolReplaceText,   // find/replace by text or hex data
      BoolReplaceAll: Boolean;
   end;

   TDialogReplace = class(TForm)
      Button1: TButton;
      Button2: TButton;
      Label1: TLabel;
      edFind: TEdit;
...
begin
   ...
   if Result then
      for LIntLoop := 1 to Length(Str) do
         if not(Str[LIntLoop] in ['0' .. '9', ' ', 'a' .. 'f', 'A' .. 'F']) then begin
            Result := False;
            Break;
         end;
end;
...
function ReplaceGetOptions(var Options: TReplaceRec): Boolean;
begin
   with TDialogReplace.Create(Application) do
      try
         ...
         if Options.StrTextToFind <> '' then cbFindText.Checked := Options.BoolFindText;
         if Options.StrTextToReplace <> '' then cbReplaceText.Checked := Options.BoolReplaceText;
         ...
         if Result then
            with Options do begin
               ...
               if cbFindText.State = cbGrayed then BoolFindText := not IsHex(edFind.Text);
               if cbReplaceText.State = cbGrayed then BoolReplaceText := not IsHex(edReplace.Text);
...
procedure TDialogReplace.edFindChange(Sender: TObject);
begin
   Button1.Enabled := edFind.Text <> '';
   Button3.Enabled := edFind.Text <> '';
end;

procedure TDialogReplace.FormCreate(Sender: TObject);
begin
   FReplaceAll := False;
end;

procedure TDialogReplace.Button3Click(Sender: TObject);
begin
   FReplaceAll := True;
end;
